...
export default class NewBill {
  ...
  /*handleChangeFile = e => {
    e.preventDefault()
    const file = this.document.querySelector(`input[data-testid="file"]`).files[0] // Gère le nommage du fichier du justificatif
  ...
  */
  ...
  handleChangeFile = (e) => {
    e.preventDefault();
    const uploadInput = this.document.querySelector(
      `input[data-testid="file"]`
    );
    const file = uploadInput.files[0];
    const fileMimeType = file.type.split("/").pop().toLowerCase(); // Récupération du type de fichier
    const filePath = e.target.value.split(/\\/g);
    const fileName = filePath[filePath.length - 1];
    const formData = new FormData();
    const email = JSON.parse(localStorage.getItem("user")).email;
    ...
    if (!["jpg", "jpeg", "png"].includes(fileMimeType)) {
      ...
      uploadInput.classList.remove("blue-border");
      uploadInput.classList.add("is-invalid");
      uploadInput.value = null;
      return;
    }

    if (uploadInput.classList.contains("is-invalid")) {
      uploadInput.classList.remove("is-invalid");
      uploadInput.classList.add("blue-border");
